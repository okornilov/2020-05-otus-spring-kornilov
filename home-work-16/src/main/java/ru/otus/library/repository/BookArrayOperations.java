...
public class BookArrayOperations implements ArrayOperations {
    ...
    @Override
    public <T extends Identifiable> void addElement(@NonNull String objectId, T element, String arrayName) {
    ...
    @Override
    public <T extends Identifiable> void updateElement(@NonNull String elementId, T element, String arrayName) {
    ...
    public <T> T findById(String arrayName, String id, Class<T> dtoClass) {
        return mongoTemplate.aggregate(newAggregation(unwind(arrayName), replaceRoot(arrayName),
                match(Criteria.where("_id").is(id))), Book.class, dtoClass).getUniqueMappedResult();
    }
